...
sub format_table_qiime_2_R { # creates an R_formatted version of a qiime table format (NOT biom)
  ...
  if ( $file_check =~ m/long/ ) {
    system("$DIR/line_term.pl -i $qiime_table_in -o $qiime_table_in.lt_edit; mv $qiime_table_in.lt_edit $qiime_table_in") or die "died runnin line_term.pl";
  } 
...
sub format_table_qiime_2_R_qiime_ID { # creates an R_formatted version of a qiime table format (NOT biom)
  ...
  my $file_check = `file $qiime_table_in`;   #check line terminators -- correct if needed
  if ( $file_check =~ m/long/ ) {
    system("$DIR/line_term.pl -i $qiime_table_in -o $qiime_table_in.lt_edit; mv $qiime_table_in.lt_edit $qiime_table_in") or die "died running line_term.pl";
  } 
...
sub format_table_R_2_qiime { # script assumes just one line of header for R format
  ...
  if ( $file_check =~ m/long/ ) {
    system("$DIR/line_term.pl -i $R_table_in -o $R_table_in.lt_edit; mv $R_table_in.lt_edit $R_table_in") or die "died running line_term.pl";
  }
