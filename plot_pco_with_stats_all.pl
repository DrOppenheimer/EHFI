...
sub process_original_qiime_data {
  ...
  if ( $qiime_format eq "qiime_table" ){ # handle qiime_table format as input
    ...
    #my $qiime_table_2_biom_string = "biom convert -i $input_dir$data_file -o $output_prefix$biom_file --table-type=\"otu table\"";
    my $qiime_table_2_biom_string = "biom convert -i $input_dir$data_file -o $output_prefix$biom_file --table-type=\"OTU table\" --to-tsv";
    ...
    my $biom_2_qiime_table_string = "biom convert -b -i $output_prefix$biom_file -o $output_prefix$qiime_table --header_key taxonomy";
...
sub process_permuted_qiime_data { # starts with biom format
  ...
  foreach my $Qiime_permutation (@Qiime_permutation_list){
    ...
    #my $qiime_table_2_biom_string = "biom convert -i $perm_dir$Qiime_permutation -o $perm_dir$biom_permutation --table-type=\"otu table\"";
    my $qiime_table_2_biom_string = "biom convert -i $perm_dir$Qiime_permutation -o $perm_dir$biom_permutation --table-type=\"OTU table\" --to-tsv";
    print $log_file "\n"."executing:"."\n".$qiime_table_2_biom_string."\n";
    system($qiime_table_2_biom_string)==0 or die "died running command"."\n".$qiime_table_2_biom_string."\n";
    #system($qiime_table_2_biom_string); # or die "died running command"."\n".$qiime_table_2_biom_string."\n";
