...
# Adapted from 8-24-12 version, for use with master that can call
# plot_pco_with_stats,
...
#my $output_prefix = "my_permutations";
...
$output_dir = $current_dir."plot_OTU_pco_with_stats.".$data_file.".".$dist_method.".RESULTS/";
...
##### HAVE TO EDIT THIS PART # DONE 8-6-12
...
#system("avg_distances.sh $data_file.$dist_method.DIST $current_dir $groups_list $data_file.$dist_method.DIST $output_dir");
...
if($debug){
  ...
  system( "cat $output_dir$perm_list | xargs -n1 -P$num_cpus -I{} OTU_similarities_shell.7-31-12.sh {} $perm_dir $output_PCoA_dir 1 $output_DIST_dir $dist_method $headers" );
}
print LOG "DONE at:"."\t".`date +%m-%d-%y_%H:%M:%S`."\n";
...
USAGE:
    ...
    --dist_method      (string)  default = $dist_method
                                    --> can slect from the following distances/dissimilarities <-- 
                                         OTU | w_OTU 
                                         OTU or abundance weighted OTU - only options for this version
                                         see plot_pco_with_stats... and plot_qiime_pco_with_stats ... for more options
    --perm_dir         (string)  default = $perm_dir
